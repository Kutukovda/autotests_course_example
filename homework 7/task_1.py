...
class Segment:
    ...
    def length(self):
        """
        :param: self.dot1 кортеж (x1, y1)
        :param: self.dot2 кортеж (x2, y2)
        :return: dlina длина отрезка с концами (x1, y1) и (x2, y2)
        """
        # d = ((x2 - x1)**2 + (y2 -y1)**2 )**0.5  формула длины отрезка по координатам
        dlina = round(((self.dot2[0] - self.dot1[0])**2 + (self.dot2[1] - self.dot1[1])**2)**0.5, 2)
        return dlina
