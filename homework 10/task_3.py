...
@pytest.mark.parametrize("set_value, result",
                         [
                                 pytest.param((60, 10, 6), 1, marks=pytest.mark.skip("плохие данные")),
                                 pytest.param((0, 6, 1), 0, marks=pytest.mark.smoke),
                                 ((30.4, 0.5), 60.8),
                         ])
def test_positive(set_value, result):
    """
    Тест all_division через parametrize позитивные сценарии
    """
...
def test_negative(set_value,type_error):
    """
    Тест all_division через parametrize негативные сценарии
    """
